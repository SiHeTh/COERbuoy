...
#Dependencies
from COERbuoy.floater_cb import Floater;
import numpy as np;
...
class Floater_BEM(Floater):    
    ...
    #calculate hydrodnymaic parameters from heave, surge, pitch, surface elevation
    def Calculate(self, z0, x0, delta0, eta):
        ...
        rad1 = [res[3][0][0]+1j*res[2][0][0],res[3][1][1]+1j*res[2][0][0],res[3][2][2]+1j*res[2][0][0]];
        am1 = [res[2][0][0],res[2][1][1],res[2][2][2]];
        
        return [fb,exc1,rad1,[am1[0][-1],am1[1][-1],am1[2][-1]]];#F_buoynacy, F_excitation, F_radiation, F_added_mass
    ...
    def get_forces(self, t, wave, z0, x0, delta0, v, a):
        ...
        am_omega = np.imag(res[2]);#added mass over omega
        am1 = np.array(res[3]);#added mass @ inf
        
        #Generate wave from movement
        if (np.sum(np.abs(exc1))>0):
            r1=am_omega[1][1]/np.conjugate(exc1[1])*(v[1]-Awave[2])+am_omega[0][1]/np.conjugate(exc1[1])*(v[0]-Awave[3]);
            r2=am_omega[0][0]/np.conjugate(exc1[0])*(v[0]-Awave[3])+am_omega[1][0]/np.conjugate(exc1[0])*(v[1]-Awave[2]);
