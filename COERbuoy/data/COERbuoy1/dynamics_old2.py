...
class WEC():
    ...
    def Calc(self,t,wave,x,PTO_force,brake,ulast):
      ...
      m_rot=np.array([[-1*np.cos(alpha), -1*np.sin(alpha)], [np.sin(alpha), np.cos(alpha)]])
      
      heave=(stroke+self.l)*np.cos(alpha)-self.l;
      surge=self.get_surge(x);
      surge_v= 0#-x[1]*np.sin(x[2]) + self.l*np.cos(x[2])*x[3];
      heave_v= 0#x[1]*np.cos(x[2]) + self.l*np.sin(x[2])*x[3];
      ...
      f_hy = self.buoy.get_forces(t,wave,[-1*surge,heave,alpha],[surge_v,heave_v,0],self.acc)
      ...
      F_sum_floater=F_radax[1]+F_gen+WS+Fd_add+brake+r1;
      ...
      print([F_radax,dx[2],dx[3],self.mass+am[0]])
