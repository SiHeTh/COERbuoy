...
class WEC():
    ...
    def Calc(self,t,wave,x,PTO_force,brake,ulast):
      ...
      f_hy = self.buoy.get_forces(t,wave,heave,surge,alpha,[0,x[1]*np.cos(x[2]),0],self.acc)
      f_hy[0][0]=f_hy[0][0];
      f_exc=f_hy[0][1]-self.mass;
      ...
      dx[6]=0;
      dx[7]=-F_gen;
      self.acc=[dx[3],dx[1],0];#*np.cos(x[2]);
      dx[8]=-Pabs;
      return dx;
